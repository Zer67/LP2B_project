...
public class ball_behavior : MonoBehaviour
{
    ...
    private readonly Vector3 Y_DIFF_WITH_PADDLE = new Vector3(0,0.38f,0);
    ...
    {   
        ...
        {   
            //For the multiple balls powerup
            if(spawner_script.getBallNumber() > 1 ) 
            {
    ...
    private void resetPosition(){  
        GetComponent<Rigidbody2D>().isKinematic = true;
        body.velocity = Vector3.zero;
        transform.position = paddle_script.transform.position + Y_DIFF_WITH_PADDLE;
        transform.parent = paddle_script.transform;
        game.ball_fall(); //Updating score
    }
    ...
    {
        GetComponent<Rigidbody2D>().isKinematic = false; //Switching off the kinematic to launch the ball
        transform.parent = null;
        powers_spawner.setSticky(false);
        body.velocity = (new Vector2(0,-y_ball_speed)).normalized * y_ball_speed;
        yield return new WaitForSeconds(0.1f); //Letting the ball launch before getting back to a sticky paddle
